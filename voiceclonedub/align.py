...
def _atempo(src, dst, factor):
    ...
    subprocess.run(["ffmpeg", "-nostdin", "-y", "-hide_banner", "-loglevel", "error",
                    "-i", src, "-af", f"atempo={factor:.4f}", dst], check=True)
    return dst


def _trim(src, dst, cut_s):
    subprocess.run(["ffmpeg", "-nostdin", "-y", "-hide_banner", "-loglevel", "error", "-i", src,
                    "-af", f"atrim=0:{cut_s:.3f},afade=t=out:st={max(0,cut_s-0.04):.3f}:d=0.04",
                    dst], check=True)
    return dst


def build_track(segs, texts, work_dir, synth_fn, max_compress=1.45, too_fast=1.18,
                loudnorm=(-16.0, -1.5, 11.0)):
    ...
    for i in range(n):
        ...
        rep.append({"i": i + 1, "start": round(st, 2), "avail": round(avail, 2),
                    "tts": round(d, 2), "final": round(fd, 2), "speed": round(speed, 3),
                    "trim_s": trim_s})

    # placement: anchor at original start; clamp to >= prev end so audio never overlaps.
    positions, placed_gap, placed_over, drifts = [], [], [], []
    prev_end = 0.0
    ...
    for _, p in pieces:
        inp += ["-i", p]
    fc = [f"[{i}:a]adelay={int(positions[i]*1000)}|{int(positions[i]*1000)}[d{i}]" for i in range(n)]
    li, ltp, llra = loudnorm                              # EBU R128 loudness normalization (else output is quiet/distant)
    fc.append("".join(f"[d{i}]" for i in range(n)) +
    ...
    subprocess.run(["ffmpeg", "-nostdin", "-y", "-hide_banner", "-loglevel", "error"] + inp +
                   ["-filter_complex", ";".join(fc), "-map", "[o]", "-c:a", "pcm_s16le", track], check=True)
