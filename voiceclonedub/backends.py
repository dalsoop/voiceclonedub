...
LANG = {"en": "English", "ja": "Japanese", "ko": "Korean", "zh": "Chinese",
...
# ---------------------------------------------------------------- HTTP helpers
def _post_json(url, payload, headers=None, timeout=180, retries=3):
...
def _chat(cfg, messages, temperature=0.2, max_tokens=2048):
    url = cfg["endpoint"].rstrip("/") + "/chat/completions"
    headers = {}
    if cfg.get("api_key"):
        headers["Authorization"] = f"Bearer {cfg['api_key']}"
    r = _post_json(url, {"model": cfg["model"], "messages": messages,
                         "temperature": temperature, "max_tokens": max_tokens}, headers)
...
def _clean(out):
    lines = [l.strip().strip('"').strip("'").strip() for l in (out or "").strip().splitlines() if l.strip()]
    lines = [l for l in lines if not re.match(r"(?i)^(okay|sure|here'?s|here is|translation)\b", l)]
    return (lines[-1] if lines else (out or "")).strip().strip('"').strip()


def _wrong_script(t, tgt):
...
def _tg(cfg, text, src_name, tgt, concise=False, force=False):
    instr = (f"Translate the following {src_name} sentence into natural {lang_name(tgt)}. "
    ...
    return _clean(_chat(cfg, [{"role": "user", "content": f"{instr}\n{text}"}],
                        temperature=0.4 if force else 0.0))


def translate_line(cfg, text, tgt, concise=False, src=None):
...
# ---------------------------------------------------------------- fidelity judge
KO_NEG = re.compile(r"(맙시다|맙시|마세요|마라|말자|말아|지\s*마|안\s|안돼|안 ?됐|안 ?돼|못\s|못해|못했|없다|없어|없습|없고|없는|없을|없으|지\s*않|지않)")
TGT_NEG = re.compile(r"(\bnot\b|n[’']?t\b|\bno\b|\bnever\b|\bnone\b|\bnor\b|\bcannot\b|\bwithout\b|\bunable\b)", re.I)


def neg_warn(segs, texts, src="ko"):
...
def judge_fidelity(cfg, segs, texts, tgt):
    ...
    rows = "\n".join(f"{i}. SRC: {segs[i-1]['text']}\n   {lang_name(tgt)}: {texts.get(i,'')}"
                     for i in range(1, len(segs) + 1))
    sysmsg = (f"You verify translation fidelity into {lang_name(tgt)} (you do NOT translate; you judge). "
    ...
    try:
        out = _chat(cfg, [{"role": "system", "content": sysmsg},
                          {"role": "user", "content": rows}], temperature=0.0, max_tokens=4000)
...
# ---------------------------------------------------------------- STT
def transcribe(cfg, audio_path, language=None):
    ...
    if backend == "faster-whisper":
        ...
        model = WhisperModel(cfg.get("model", "large-v3"),
...
# ---------------------------------------------------------------- TTS
def tts(cfg, text, out_bytes_path, voice_ref=None):
    ...
    body = {"input": text, "model": cfg.get("model", "voxcpm"),
            "cfg_value": cfg.get("cfg", 1.9), "inference_timesteps": cfg.get("steps", 40)}
    if voice_ref:
    ...
    req = urllib.request.Request(endpoint.rstrip("/") + "/v1/audio/speech", data=data, headers=headers)
