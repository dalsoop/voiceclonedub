...
from . import srt, merge, align, backends, media
...
def _coverage(track, lang, segs, texts, stt_cfg):
    ...
    for i in range(1, len(segs) + 1):
        ...
        if len(a) < 3:
            continue
        best = max((difflib.SequenceMatcher(None, a, heard[j:j + len(a) + 8]).ratio()
                    for j in range(0, max(1, len(heard) - len(a)), 4)), default=0)
        if best < 0.62:
            miss.append(i)
    return round(1 - len(miss) / max(1, len(segs)), 2), miss


def _translate_all(tcfg, segs, tgt, idxs=None, concise=False, src=None):
    pick = idxs or list(range(1, len(segs) + 1))
    out = {}
    for i in pick:
        try:
            out[i] = backends.translate_line(tcfg, segs[i - 1]["text"], tgt, concise=concise, src=src)
        except Exception:  # noqa: BLE001
            out[i] = ""
    return out


def dub(video, tgt, cfg, src=None, voice=None, rounds=3, out_dir="out", work_root="work"):
    ...
    m = cfg["merge"]
    segs = merge.merge(raw_cues, break_gap=m["break_gap"], hard_gap=m["hard_gap"],
                       max_dur=m["max_dur"], max_chars=m["max_chars"], min_dur=m["min_dur"])
    if not segs:
    ...
    def synth_fn(idx, text):
        key = hashlib.sha1(f"{text}|{voice}|{tts_cfg.get('model')}|{tts_cfg.get('cfg')}|{tts_cfg.get('steps')}".encode()).hexdigest()
    ...
    for rd in range(rounds):
        track, last = align.build_track(segs, texts, work, synth_fn,
                                        max_compress=a["max_compress"], loudnorm=ln)
        fast = set(last["too_fast"]) | set(last["overlap"]) | set(last["word_trimmed"])
        if not fast:
            break
        texts.update(_translate_all(cfg["translate"], segs, tgt, idxs=sorted(fast), concise=True, src=src))
    ...
    ok = (not empty and not last["overlap"] and not last["too_fast"]
          and not last["word_trimmed"] and last["drift_max"] <= a["max_drift_s"]
          and cov >= a["min_coverage"] and loud_ok)
    gates = {"empty": empty, "fidelity_errors": fidelity, "neg_warn_info": nwarn,
             "overlap": last["overlap"], "too_fast": last["too_fast"],
             "word_trimmed": last["word_trimmed"], "coverage": cov, "coverage_miss": cov_miss,
             "drift_max": last["drift_max"], "big_gap_info": last["big_gap"],
    ...
    artifact_ok = (os.path.exists(out_path) and os.path.getsize(out_path) > 10000
                   and "video" in sv and "audio" in sv and out_dur > 1.0)
    gates["artifact"] = {"out_exists": os.path.exists(out_path), "has_video": "video" in sv,
                         "has_audio": "audio" in sv, "dur_s": round(out_dur, 1), "ok": artifact_ok}
    ok = ok and artifact_ok
    record = {"stamp": stamp, "slug": slug, "src": src, "tgt": tgt, "segs": len(segs),
              "rounds": rd + 1, "ok": ok, "gates": gates, "out": out_path,
              "lines": [{"start_ms": s["start_ms"], "text": texts.get(i + 1, "")}
